    ...
    <div style={{ fontFamily: "Inter, sans-serif" }} className="animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <table className="w-full text-sm border-collapse">
            ...
            <thead>
              <tr className={`${isDark ? "bg-white/5 text-gray-400 border-b border-white/10" : "bg-gray-200 text-black border-b border-black/10"}`}>
                ...
                <th className="px-6 py-4 font-normal text-left text-[13px]  ">Customer</th>
                <th className="px-6 py-4 font-normal text-left text-[13px] ">Commission Type</th>
                <th className="px-6 py-4 font-normal text-left text-[13px] ">Pricing</th>
                <th className="px-6 py-4 font-normal text-center text-[13px] ">Status Flow</th>
                <th className="px-6 py-4 font-normal text-center text-[13px] ">Received</th>
                <th className="px-6 py-4 font-normal text-right text-[13px] ">Actions</th>
              </tr>
            </thead>
                ...
                >
                  ...
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg ${isDark ? "bg-white/5 text-white" : "bg-black/5 text-black"}`}>
                        {order.name.charAt(0)}
                      </div>
                      <div className="flex flex-col">
                        <span className={`text-[15px] ${isDark ? "text-white" : "text-neutral-900"}`}>{order.name}</span>
                        <span className={`text-[12px] font-medium opacity-50`}>{order.email}</span>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                      <span className={`font-black text-lg ${isDark ? "text-white" : "text-black"}`}>
                        ₹{order.totalPrice.toLocaleString()}
                      </span>
                      <span className="text-[10px]  opacity-60 uppercase">Advance: ₹{order.advanceAmount}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-5 text-center">
                    <div className="flex flex-col">
                      ...
                      <span className="text-[12px] opacity-30 font-medium">
                  ...
                  <td className="px-6 py-5 text-right">
                    <div className="flex justify-end gap-3 translate-x-2 transition-all duration-300">
                       <button 
                         onClick={() => setSelectedOrder(order)}
                         title="Detailed Audit"
                         className={`p-2.5  transition-all  hover:scale-110  ${isDark ? " text-blue-400 hover:text-blue-500 " : " text-blue-600 hover:text-blue-700"}`}>
                         <Eye size={18} />
                       </button>
                       <button 
                         onClick={() => deleteOrder(order._id)}
                         title="Archive Order"
                         className={`p-2.5 rounded-lg transition-all  hover:scale-110 active:scale-90 ${isDark ? " text-red-400 hover:text-red-500 " : " text-red-600 hover:text-red-600 "}`}>
                         <Trash2 size={18} />
                       </button>
                    </div>
                  </td>
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/90 backdrop-blur-md animate-in fade-in duration-500">
          <div 
            className={`w-full max-w-4xl max-h-[92vh] overflow-hidden rounded-2xl border shadow-[0_0_100px_rgba(0,0,0,0.5)] animate-in zoom-in-95 duration-500 flex flex-col
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-10 custom-scrollbar">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12">
                        ...
                        {(selectedOrder.history && selectedOrder.history.length > 0) ? (
                          selectedOrder.history.map((item, idx) => (
                            <div key={idx} className="relative animate-in slide-in-from-left-4 duration-500" style={{ animationDelay: `${idx * 100}ms` }}>
                               ...
                               <div>
                                  <p className="text-sm font-bold uppercase tracking-wide">{item.status.replaceAll('_', ' ')}</p>
                ...
                <div className="lg:col-span-5 space-y-10">
                  ...
                  <div className={`p-8 rounded-lg border transition-all duration-500 shadow-2xl relative overflow-hidden
                    ${selectedOrder.transactionId 
                      ? "bg-indigo-600/10 border-indigo-500/30 text-indigo-400" 
                      : isDark ? "bg-white/5 border-white/10 opacity-30" : "bg-black/5 border-black/10 opacity-30"}`}>
                    
                    <div className="relative z-10">
                       <h4 className="text-[10px] uppercase opacity-80 mb-2 flex items-center gap-2">
                          <IndianRupee size={12} /> Payment Reference
                       </h4>
                       <p className={`text-xl font-black ${selectedOrder.transactionId ? "text-indigo-400" : "text-gray-500"}`}>
                       ...
                       {selectedOrder.transactionId && (
                         <button 
                           ...
                           className="mt-4 flex items-center gap-2 text-[10px] uppercase bg-indigo-500 text-white px-4 py-2 rounded-lg hover:bg-indigo-400 transition-all shadow-lg shadow-indigo-500/20">
                           <MoreVertical size={14} className="rotate-90" /> Copy for verification
                         </button>
                       )}
                    </div>
                    {selectedOrder.transactionId && (
                          ...
                          <div className="space-y-4">
                            <div className="flex justify-between items-center bg-transparent backdrop-blur-sm p-4 rounded-xl border border-white/5">
                              ...
                              {selectedOrder.transactionId && !selectedOrder.isAdvancePaid && (
                                 <button 
                                   onClick={() => updateStatus(selectedOrder._id, 'payment_done')}
                                   className="px-3 py-1 bg-emerald-500 text-white text-[10px] font-black rounded-full uppercase hover:scale-105 transition-transform">
                                    Verify Advance
                                 </button>
                              )}
                              {selectedOrder.isAdvancePaid && <span className="text-emerald-500">✅</span>}
                            </div>
                            ...
                            <div className="flex justify-between items-center bg-transparent backdrop-blur-sm p-4 rounded-xl border border-white/5">
                              ...
                              {selectedOrder.balanceTransactionId && !selectedOrder.isFullPaid && (
                                 <button 
                                   ...
                                   className="px-3 py-1 bg-sky-500 text-white text-[10px] font-black rounded-full uppercase hover:scale-105 transition-transform">
                                    Verify & Ship
                                 </button>
                              )}
                              {selectedOrder.isFullPaid && <span className="text-emerald-500">✅</span>}
                            </div>
