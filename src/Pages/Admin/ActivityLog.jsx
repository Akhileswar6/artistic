      ...
      <div className={`rounded-2xl border overflow-hidden shadow-2xl transition-all duration-300 ${isDark ? "bg-[#0a0a0a] border-white/10" : "bg-white border-black/5"}`}>
        ...
        <div className={`p-4 border-b ${isDark ? "border-white/5" : "border-black/5"}`}>
          <div className="relative w-full">
            ...
            <input
              ...
              className={`pl-11 pr-4 py-3 rounded-xl text-sm w-full transition-all focus:outline-none 
                ${isDark 
                  ? "bg-black border border-white/10 text-white focus:border-white/30" 
                  : "bg-gray-50 border border-black/10 text-black focus:border-black/30 shadow-sm"}`}
            />
          </div>
        </div>
            ...
            <div key={act._id} className="p-4 flex flex-col gap-3">
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold ${isDark ? "bg-blue-500/10 text-blue-400" : "bg-blue-50 text-blue-600"}`}>
                    {act.adminName.charAt(0)}
                  </div>
                  <div className="flex flex-col">
                    <span className={`text-sm font-bold ${isDark ? "text-white" : "text-black"}`}>{act.adminName}</span>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      {getActionIcon(act.action)}
                      <span className={`text-[10px] px-2 py-0.5 rounded-full uppercase font-bold ${isDark ? "bg-white/5 text-gray-400" : "bg-black/5 text-gray-600"}`}>
                        {act.action}
                      </span>
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-between">
                <span className={`text-[9px] uppercase tracking-widest font-bold ${isDark ? "text-blue-400" : "text-blue-600"}`}>
        ...
        <div className="hidden lg:block overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className={`${isDark ? "bg-white/[0.02] text-gray-400 border-b border-white/5" : "bg-gray-200 text-black border-b border-black/10"}`}>
                <th className="px-6 py-4 font-normal text-[13px] uppercase tracking-wider">Admin</th>
                <th className="px-6 py-4 font-normal text-[13px] uppercase tracking-wider">Event</th>
                <th className="px-6 py-4 font-normal text-[13px] uppercase tracking-wider">Target</th>
                <th className="px-6 py-4 font-normal text-[13px] uppercase tracking-wider">Activity Details</th>
                <th className="px-6 py-4 text-right font-normal text-[13px] uppercase tracking-wider">Timestamp</th>
              </tr> 
            </thead>
                ...
                <tr key={act._id} className={`${isDark ? "hover:bg-white/[0.02]" : "hover:bg-black/[0.01]"} transition-colors group`}>
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                       ...
                       <span className={`text-sm font-medium ${isDark ? "text-gray-200" : "text-black"}`}>{act.adminName}</span>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-2">
                       {getActionIcon(act.action)}
                       <span className={`text-[11px] px-2.5 py-1 rounded-full uppercase font-bold ${isDark ? "bg-white/5 text-gray-400" : "bg-black/5 text-gray-600"}`}>
                          {act.action}
                       </span>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <div className="flex flex-col">
                      {act.targetType && act.targetType.toLowerCase() !== "order" && (
                        <span className={`text-[10px] font-bold ${isDark ? "text-blue-400/80" : "text-blue-600"}`}>{act.targetType}</span>
                      )}
                      <span className={`text-[14px]  ${isDark ? "text-blue-400" : "text-blue-600"}`} style={{fontFamily: "'Fira Code', monospace"}}>
                         #{act.targetId?.slice(-10) || "SYSTEM"}
                      </span>
                    </div>
                  </td>
                  <td className="px-6 py-5">
                    <p className={`text-[12px] font-medium max-w-[400px] leading-relaxed ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                       {act.details}
                    </p>
                  </td>
                  <td className="px-6 py-5 text-right">
                    <div className="flex flex-col items-end">
                       <span className={`text-xs font-bold ${isDark ? "text-gray-300" : "text-black"}`}>{new Date(act.timestamp).toLocaleDateString()}</span>
                       <span className= {`text-[10px] opacity-40 uppercase tracking-tighter font-bold ${isDark ? "text-blue-400" : "text-blue-600"}`}>{new Date(act.timestamp).toLocaleTimeString()}</span>
                    </div>
                  </td>
                </tr>
              ))}
