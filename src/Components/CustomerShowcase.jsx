...
export default function CustomerShowcase({ isDark }) {
  ...
  return (
    <section
      ref={sectionRef}
      className={`py-12 md:py-24 px-4 md:px-8 transition-colors duration-500 ${isDark ? "bg-black" : "bg-white"
        }`}
    >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-6">
          {showcaseItems.map((item, index) => (
            <motion.div
              ...
              className="group"
            >
              <div className={`relative aspect-[3/4] overflow-hidden rounded-2xl transition-all duration-500 border ${isDark ? "bg-neutral-900 border-white/5" : "bg-neutral-50 border-black/5"
                }`}>
                <img
                  src={item.image}
                  alt={item.title}
